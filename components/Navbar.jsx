...
const Navbar = () => {
    ...
    const handleNav = () => {
        // change state from true to false or vice versa
        setNav(!nav)
    };
    ...
    return (
      <div style={{backgroundColor: `${color}`}} className='fixed left-0 top-0 w-full z-10 eas-in duration-300'>
        <div className='max-w-[1240px] m-auto flex justify-between items-center p-4 text-white'>
          <Link href="/">
            <h1 style={{color: `${textColor}`}} className='font-bold text-4xl'>Captur</h1>
          </Link>
          <ul style={{color: `${textColor}`}} className='hidden sm:flex className='>
            <li className='p-4'>
              <Link href="/">Home</Link>
            </li>
            <li className='p-4'>
